...
start "DebugConsole" cmd /c "call conda activate D:\chainlit\molmo_env && minio server D:\chainlit\chainlit-datalayer\demo_app --console-address :9001"

REM 启动lightrag_server（后台运行）使用相同窗口标题
echo Starting lightrag_server...
start "DebugConsole" cmd /c "call conda activate D:\chainlit\molmo_env && python D:\chainlit\chainlit-datalayer\demo_app\lightrag_server.py"
